...
class FinancialStatementsInput(BaseToolInput):
    ...
    statement_types: List[Literal["income", "balance", "cash"]] = Field(
        default=["income"], description="Types of financial statements to retrieve"
    )
    frequency: Literal["quarterly", "annual"] = Field(
        default="quarterly", description="Reporting frequency"
    )
    max_periods: int = Field(
        default=8, description="Maximum number of periods to return", ge=1, le=20
    )
...
class FinancialStatementsOutput(BaseModel):
    ...
    statements: Dict[str, str] = Field(
        description="Dictionary mapping statement type to CSV formatted data"
    )
