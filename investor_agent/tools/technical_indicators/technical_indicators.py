...
class TechnicalIndicatorsTool(Tool):
    ...
    async def execute(self, input_data: TechnicalIndicatorsInput) -> ToolResponse:
        ...
        try:
            ...
            history = yf_call(ticker, "history", period=input_data.period, interval="1d")
            ...
            if len(close_prices) < min_required:
                raise ValueError(f"Insufficient data for {input_data.indicator} ({len(close_prices)} points, need {min_required})")

            # Calculate indicators using mapping
            indicator_funcs = {
                "SMA": lambda: {"sma": talib.SMA(close_prices, timeperiod=input_data.timeperiod)},
                "EMA": lambda: {"ema": talib.EMA(close_prices, timeperiod=input_data.timeperiod)},
                "RSI": lambda: {"rsi": talib.RSI(close_prices, timeperiod=input_data.timeperiod)},
                "MACD": lambda: dict(zip(["macd", "signal", "histogram"],
                    talib.MACD(close_prices, fastperiod=input_data.fastperiod,
                              slowperiod=input_data.slowperiod, signalperiod=input_data.signalperiod))),
                "BBANDS": lambda: dict(zip(["upper_band", "middle_band", "lower_band"],
                    talib.BBANDS(close_prices, timeperiod=input_data.timeperiod,
                               nbdevup=input_data.nbdev, nbdevdn=input_data.nbdev, matype=MA_Type(input_data.matype))))
            }
            ...
            for i, date in enumerate(price_df['Date']):
                ...
                for name, values in indicator_values.items():
                    ...
                    slice_values = values[-input_data.num_results:] if input_data.num_results > 0 else values
            ...
            error_msg = f"Error calculating {input_data.indicator} for {input_data.ticker}: {e}"
            output = TechnicalIndicatorsOutput(data={"error": error_msg})
            return ToolResponse.from_model(output)
